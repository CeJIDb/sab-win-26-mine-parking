...
- [x] **Фаза 3. Регламент взаимодействия ИС v1 (структура).** Добавлен Блок 3 «Документооборот через ЭДО» в [is-interaction-regulation.md](../docs/architecture/integration/is-interaction-regulation.md) по образцу Блока 2: 10 строк таблицы (6 по INT-014 + 4 по INT-015), колонки «Протокол» и «Полезная нагрузка» — `уточняется` (заполним в Фазе 9). Внутренние INT-016 / INT-017 в регламент не вошли — будут раскрыты в UML Sequence (Фаза 4). Терминология «Сервис Договоров» (а не «Договор») — синхронизация с C4 / DFD L1.
...
- **Терминология «Сервис Договоров» вместо «Договор»** — в колонках источника / приёмника таблицы регламента и в архитектурном допущении используются имена контейнеров C4 / DFD L1. Будущие архитектурные артефакты UC-8.2 (Sequence Фазы 4, OpenAPI Фазы 8, регламент Фазы 9) — пишут «Сервис Договоров». В файле требований [integration-requirements.md](../docs/specs/integration/integration-requirements.md) сохраняется логическое имя «Договор» (формат `INT-*`), это допустимое расхождение между уровнями спецификаций и архитектуры.
- **Внутренние интеграции INT-016 / INT-017 в регламент не выносятся** — они описаны только в требованиях и раскроются в UML Sequence (Фаза 4). На уровне регламента отображаются только направления обмена с **внешними** системами (ЭДО и Сервис уведомлений). Локальный ЭДО Adapter упомянут в шапке Блока 3 как учебное допущение.
