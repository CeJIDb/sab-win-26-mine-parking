...
- Фаза 1 (UC-8.2) → `2026-MM-DD-uc-8-2-document.md`
...
- [ ] **Фаза 1. UC-8.2 — детальный сценарий.** Создать `docs/artifacts/use-case/uc-8-2-create-contract-legal-entity.md` по структуре uc-10-2: назначение, контекст, акторы, предусловия, постусловия, основной поток, альтернативные потоки, исключения. Включить пометку про локальный ЭДО Adapter в шапке.
