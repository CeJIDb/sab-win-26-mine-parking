...
- [Таблица `agreement_types`](#таблица-agreement_types)
...
- [Таблица `benefit_categories`](#таблица-benefit_categories)
...
| `channel` | `notification_channel_enum` | NOT NULL | значения: `SMS`, `EMAIL`, `PUSH`; не id; входит в состав `PRIMARY KEY` |
| `created_at` | `TIMESTAMPTZ` | NOT NULL | `DEFAULT now()` |
| `updated_at` | `TIMESTAMPTZ` | NOT NULL | `DEFAULT now()`; обновление триггером `moddatetime` |
...
## Таблица `agreement_types`

Схема: `client` (словарная).
...
- `clients.type` — `client_type_enum` со значениями `FL` / `UL`.
