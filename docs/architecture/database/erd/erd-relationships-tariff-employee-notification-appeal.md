...
- [Таблица `tariff_types`](#таблица-tariff_types)
...
| `benefit_category_id` | `BIGINT` | NULL | логическая ссылка на `client.benefit_categories(id)` (без `REFERENCES`, ADR-003) |
...
## Таблица `tariff_types`

Схема: `tariff` (словарная).
...
| `subject_type` | `subject_entity_enum` | NULL | значения: `BOOKING`, `SESSION`, `PAYMENT`, `RECEIPT`, `CONTRACT` |
...
- `tariff.tariffs.benefit_category_id -> client.benefit_categories.id`
