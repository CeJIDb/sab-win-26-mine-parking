...
| ERD-069 | +9 индексов: `sector(parking_id)`, `access_points(parking_id)`, `parking_place(sector_id)`, `parking_session(entry/exit_access_point_id)`, `client_account(client_id)`, `access_log(access_point_id/decided_at/vehicle_id)`; исправлен комментарий `parking_schedule(parking_id)` |
...
**ACCESS_LOG:**
- Append-only; INSERT only. `access_point_id NOT NULL`; `vehicle_id`, `client_id` — nullable (нераспознанный ГРЗ).
...
| 6–7 | ERD-057 | `ACCESS_LOG` добавлен в схему `report`: `access_point_id`, `vehicle_id`/`client_id` (nullable), `direction CHECK('IN','OUT')`, `decision CHECK('ALLOW','DENY','MANUAL')`, `reason`, `decided_at`; append-only |
...
| 8 | ERD-069 | Секция 5: добавлены индексы `sector(parking_id)`, `access_points(parking_id)`, `parking_place(sector_id)`, `parking_session(entry/exit_access_point_id)`, `client_account(client_id)`, `access_log(access_point_id/decided_at/vehicle_id)`; исправлен комментарий к `parking_schedule(parking_id)` |
