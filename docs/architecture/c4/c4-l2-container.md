...
| **Основной процесс (API)** | `[Container]` | Модульный монолит | Основной backend: REST API, `Application Service`, 14 внутренних контекстов, включая `Расчет стоимости` и `Адаптер дисплеев и табло`, бизнес-правила парковки |
...
> На Level 2 `Application Service`, `Access`, `Booking`, `Session`, `Tariff`, `Calculation`, `Payment` и остальные внутренние bounded contexts не выделяются в отдельные контейнеры, потому что исполняются внутри **одного** контейнера `Основной процесс (API)`. В русскоязычной терминологии `Calculation` соответствует контексту `Расчет стоимости`. Исключение, `Адаптер СКУД`, он вынесен отдельно из-за транспорта UDP и vendor-specific интеграции.
