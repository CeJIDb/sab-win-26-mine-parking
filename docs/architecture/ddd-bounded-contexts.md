...
Документ уточняет модульную декомпозицию из [ADR-003](adr-003-modular-monolith-c.md) в терминах DDD (Domain-Driven Design) и фиксирует, какие **bounded contexts** (ограниченные контексты) считаются изолированными внутри целевого модульного монолита.
...
- Решение `allow/deny` принимается **онлайн платформой**, а не СКУД (системой контроля и управления доступом), согласно [ADR-001](adr-001-online-access-rights-evaluation.md).
...
- Бронирование является обязательной основой для `Session`, согласно [ADR-002](adr-002-booking-vs-session.md).
...
- [ADR-001](adr-001-online-access-rights-evaluation.md) — оценка прав доступа онлайн на каждый запрос КПП
- [ADR-002](adr-002-booking-vs-session.md) — Бронирование vs Парковочная сессия как мастер-сущности
- [ADR-003](adr-003-modular-monolith-c.md) — выбор архитектурного стиля: модульный монолит
